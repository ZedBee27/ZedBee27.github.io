...
const DifficultyLevelQuestionTableUser = ({ title, limit, difficultyLevel }: QuestionTableProps) => {
    ...
    const difficultyLevelQuestions = questions.filter((question) => question.difficulty.toLowerCase() === difficultyLevel?.toLowerCase());

    const lastQuestionIndex = currentPage * questionsPerPage;
    const firstQuestionIndex = lastQuestionIndex - questionsPerPage;
    const currentQuestions = difficultyLevelQuestions.slice(firstQuestionIndex, lastQuestionIndex);
    ...
    return ( 
        <div className="mt-10">
                ...
                <TableBody>
                    {currentQuestions.map((question) => (
                        <TableRow key={question.id} className="odd:bg-blue-50 dark:odd:bg-slate-950">
                            ...
                            <TableCell className="hidden md:table-cell dark:text-blue-600">{question.topic}</TableCell>
                            <TableCell className="flex flex-row dark:text-blue-600">
                            {question.type === 'MCQ' && (
                                    <Link href={`/user/difficultylevels/${difficultyLevel}/practiceMCQ/${question.id}`}>
                                        <Button
                                ...
                                {question.type === 'DESCRIPTIVE' && (
                                    <Link href={`/user/difficultylevels/${difficultyLevel}/practiceDescriptive/${question.id}`}>
                                        <Button
                                            onClick={() => analytics.track('Going to Practice Page', {
                                                question_id: question.id,
                                                question: question.question,
            ...
            <Pagination
                totalItems={difficultyLevelQuestions.length}
...
export default DifficultyLevelQuestionTableUser;