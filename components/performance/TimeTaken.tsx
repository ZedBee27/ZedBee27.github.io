...
const TimeTakenChart: React.FC<AverageScoresChartProps> = ({userID}) => {
    ...
    const chartData = data.map((entry, index) => ({
        ...
        timeTaken: entry._avg.timeTaken,
        totalMarks: 100,
        fill: COLORS[0],
    }));
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>Average Time Spend</CardTitle>
                <CardDescription>
                    Distribution of average time spend across different exams
                </CardDescription>
            </CardHeader>
            <CardContent>
                <ChartContainer config={{}} className="mx-auto aspect-[2/1] max-h-[250px]">
                    <BarChart width={800} height={400} data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <Legend />
                        <Bar dataKey="timeTaken" fill="#0088FE">
                            <LabelList dataKey="timeTaken" position="top" />
                        </Bar>
                    </BarChart>
                </ChartContainer>
            </CardContent>
